...
plt.plot(harmonic_num,color='blue')
plt.xlabel('time')
plt.ylabel('# records')
...
for block in blocks:
    sample_size=10000
    random.seed(0)
    MAD_D = MAD([sum(random.choices(negLogReturns,k=1)) for idx in range(sample_size)])
    
    MAD_N = MAD([sum(random.choices(negLogReturns,k=block)) for idx in range(sample_size)])

    kappa_i = 2 - (np.log(block) - np.log(1))/np.log(MAD_N/MAD_D)
...
for block in blocks:
    ...
    sample_size=10000
    random.seed(0)
    MAD_D = MAD([sum(random.choices(posLogReturns,k=1)) for idx in range(sample_size)])
    
    MAD_N = MAD([sum(random.choices(posLogReturns,k=block)) for idx in range(sample_size)])
